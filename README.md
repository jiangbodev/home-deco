...
当前入口是 `public/assets/modules/manifest.json`，不是历史单文件 `home.glb`。以下数据对应本轮全屋走查后的发布版本。
...
| Mesh 节点 | 1,386 |
| 入口模型及贴图 | 约 3.45 MB |
| 全部模块、贴图及清单 | 约 6.87 MB |
| 加上 Cycles 光照、HDR 反射及表面资源 | 约 12.06 MB |
| 资源预算 | 小于 20 MB |
...
本地最新调整：1.4 m 视角、两张床的纯深藏青整床被、去除过时沙发 AO、提亮木柜、删除壁龛两块多余竖板、统一弧面和直面的光照；餐桌花瓶及花枝已移除，主卫内部压纹隔断默认打开、可手动开关；底部导航含主卫和次卫。窗外两棵树向外 1.8 m、向上 0.9 m。自动画质更早响应掉帧，限制绘制像素数并降低玻璃透射目标分辨率。见 [最新本地记录](review/comfort-local-2026-09-21.md) 和 [实机截图](review/comfort-details.jpg)。

## 自然光与材质拟真

Blender Cycles 对现有完整户型计算日光、窗光遮挡、五次漫反射反弹和实际餐桌灯位置的暖光。7 块地面与 509 个墙柜等表面使用离线光照图；每个表面独立降噪，避免图集串色。窗边到室内、桌下和椅脚的亮暗关系更明显，柜面仍保留米白色。

客厅、主卧分别使用同一 Blender 场景生成的局部 HDR 反射，经一次预过滤后作用于漆面、木面、台面和金属。漫游时不增加实时阴影、反射或全屏 AO 渲染。当前模型为 532,981 个三角形，入口模型资源约 3.45 MB；新增光照在入口可用后后台加载。
...
本地最新全屋走查：见 [九房间画面、交互及性能记录](review/whole-house-latest.md)。本轮修改已获用户确认提交。
