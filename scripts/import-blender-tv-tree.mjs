...
for(const key of Object.keys(manifest.modules)){
 ...
 for(const n of doc.getRoot().listNodes()){
  ...
  for(let i=0;i<oldPrimitives.length;i++){
   ...
   report.push({module:key,name:n.getName(),before,after,oldBounds,newBounds,method:'Blender TV head and woody tree repair; original module hierarchy/materials preserved'});
  }
 }
...
await fs.writeFile(output+'/manifest.json',JSON.stringify(manifest,null,2)+'\n');await fs.writeFile('review/tv-tree-integration.json',JSON.stringify(report,null,2)+'\n');console.log(report);
