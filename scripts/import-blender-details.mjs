// Transfer authored detail meshes/materials into stable modular node identities.
import{NodeIO,PropertyType}from'@gltf-transform/core';import{ALL_EXTENSIONS}from'@gltf-transform/extensions';import{copyToDocument,prune,unpartition,draco}from'@gltf-transform/functions';import*as T from'three';import D from'draco3dgltf';import fs from'node:fs/promises';import crypto from'node:crypto';import sharp from'sharp';import assert from'node:assert/strict';
...
for(const[key,entry]of Object.entries(manifest.modules)){
 ...
 for(const n of doc.getRoot().listNodes()){
  if(!n.getMesh()||!changed.has(n.getName()))continue;const src=authored.get(n.getName());assert(src?.getMesh(),'Missing '+n.getName());const target=key==='base'?n:parents.get(n.getExtras().attachTo),matrix=new T.Matrix4().fromArray(target.getWorldMatrix()).invert().multiply(new T.Matrix4().fromArray(src.getWorldMatrix())),normal=new T.Matrix3().getNormalMatrix(matrix),mesh=doc.createMesh(n.getMesh().getName());
  const before=n.getMesh().listPrimitives().reduce((s,p)=>s+p.getIndices().getCount()/3,0);
  for(const p of src.getMesh().listPrimitives()){
   const map=copyToDocument(doc,edited,[...p.listAttributes(),p.getIndices()]),q=doc.createPrimitive().setIndices(map.get(p.getIndices())).setMaterial(material(p.getMaterial()));
   for(const semantic of p.listSemantics()){const a=map.get(p.getAttribute(semantic));q.setAttribute(semantic,a);if(['POSITION','NORMAL','TANGENT'].includes(semantic)){const data=a.getArray(),stride=a.getElementSize(),v=new T.Vector3();for(let j=0;j<a.getCount();j++){v.fromArray(data,j*stride);if(semantic==='POSITION')v.applyMatrix4(matrix);else v.applyMatrix3(normal).normalize();v.toArray(data,j*stride)}}}mesh.addPrimitive(q)
  }
  n.setMesh(mesh);report.push({module:key,name:n.getName(),before,after:mesh.listPrimitives().reduce((s,p)=>s+p.getIndices().getCount()/3,0)});
 }
 await doc.transform(prune({propertyTypes:[PropertyType.MESH,PropertyType.PRIMITIVE,PropertyType.ACCESSOR,PropertyType.MATERIAL,PropertyType.TEXTURE]}),unpartition(),draco({quantizePosition:16,quantizeNormal:10,quantizeTexcoord:12,encodeSpeed:0,decodeSpeed:0}));
 const{json,resources}=await io.writeJSON(doc,{format:'GLTF'});assert.equal(json.buffers.length,1);const binary=Buffer.from(resources[json.buffers[0].uri]);delete json.buffers[0].uri;
 for(const image of json.images??[]){const f=[...fresh.values()].find(f=>f.file===image.uri);assert.deepEqual(Buffer.from(resources[image.uri]),f?.data??await fs.readFile(input+'/'+image.uri))}entry.textures=[...new Set((json.images??[]).map(i=>i.uri))];
 const str=Buffer.from(JSON.stringify(json)),j=Buffer.alloc(Math.ceil(str.length/4)*4,32),b=Buffer.alloc(Math.ceil(binary.length/4)*4);str.copy(j);binary.copy(b);const h=Buffer.alloc(20),bh=Buffer.alloc(8);h.writeUInt32LE(0x46546c67,0);h.writeUInt32LE(2,4);h.writeUInt32LE(28+j.length+b.length,8);h.writeUInt32LE(j.length,12);h.writeUInt32LE(0x4e4f534a,16);bh.writeUInt32LE(b.length,0);bh.writeUInt32LE(0x004e4942,4);const raw=Buffer.concat([h,j,bh,b]),file=key+'-'+crypto.createHash('sha256').update(raw).digest('hex').slice(0,12)+'.glb';await fs.writeFile(output+'/'+file,raw);manifest.modules[key]={...entry,file,bytes:raw.length};
}
const used=new Set(Object.values(manifest.modules).flatMap(m=>m.textures));for(const f of Object.keys(manifest.textures))if(!used.has(f))delete manifest.textures[f];await fs.writeFile(output+'/manifest.json',JSON.stringify(manifest,null,2)+'\n');await fs.writeFile(reportFile,JSON.stringify(report,null,2));console.log(report);
