...
async function start(){
 try{
   ...
   const gltf=await loader.loadAsync(import.meta.env.BASE_URL+'assets/home.glb',e=>{if(e.total){const pct=Math.round(e.loaded/e.total*100);$('#progress').value=pct;$('#load-label').textContent=pct<100?`正在下载空间模型 · ${pct}%`:'正在展开模型与材质…'}});
