...
export function createSurfaceFinishes(renderer){
 ...
 function prepare(group){group.traverse(o=>{
  ...
  const result=materials.map(original=>{
   ...
   const profile=[25,32,35,41,45,320].includes(id)?{...profiles.timber,rough:[25,32,35,320].includes(id)?.48:.55}:profiles[kind],edge=profile.edge&&boxEasing(o.geometry)?profile.edge:0;
   ...
   m.onBeforeCompile=(shader,...args)=>{
    ...
    shader.fragmentShader=declarations+'uniform sampler2D finishGrain;\n'+(edge?'uniform vec3 finishMin;\nuniform vec3 finishMax;\n':'')+shader.fragmentShader
     ...
     .replace('#include <roughnessmap_fragment>',`#include <roughnessmap_fragment>
      ...
      ${kind==='timber'&&m.map?'roughnessFactor=clamp(roughnessFactor+(dot(sampledDiffuseColor.rgb,vec3(.2126,.7152,.0722))-.3)*.1,'+(id===320?'.36':[25,32,35,41,45].includes(id)?'.42':'.52')+',.9);':''}
     `)
   ...
   m.customProgramCacheKey=()=>previousKey+'|finish-v6-'+kind+([25,32,35,41,45,320].includes(id)?'-satin-'+id:'')+(edge?'-edge':'');return m;
