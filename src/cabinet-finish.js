...
export function prepareCabinetFinish(group){group.traverse(o=>{
 const livingWood=softLivingWood.has(o.name),name=o.userData.source_name||o.name,printedArt=/^(展示画框画芯|画芯线条示意)/.test(name),recessBacking=name==='南侧通高柜2035门后暗缝底';
 if(!o.isMesh||seen.has(o)||(!names.test(name)&&!entryJoinery.has(name)&&!printedArt&&!recessBacking&&!livingWood))return;seen.add(o);
 const multi=Array.isArray(o.material);const materials=(multi?o.material:[o.material]).map(original=>{
  if(!printedArt&&!recessBacking&&original.userData.source_material_id!==(livingWood?370:38))return original;
  ...
  m.onBeforeCompile=(shader,...args)=>{
   ...
   shader.fragmentShader=shader.fragmentShader.replace('#include <aomap_fragment>',`vec3 cabinetContinuousDiffuse=reflectedLight.directDiffuse+reflectedLight.indirectDiffuse;\n#include <aomap_fragment>`)
    .replace('vec3 totalSpecular =',`totalDiffuse=mix(totalDiffuse,mix(cabinetContinuousDiffuse,totalDiffuse,${printedArt?'0.0':livingWood?'0.40':diningFlat?'0.55':'0.16'}),${recessBacking?'cabinetBackingMask':'1.0'});\nvec3 totalSpecular =`);
  };
  m.customProgramCacheKey=()=>key+'|continuous-cabinet-lacquer-v3'+(livingWood?'|soft-living-oak':'')+(diningFlat?'|dining-flat':'')+(printedArt?'|printed-paper':recessBacking?'|ivory-lower-backing':'');return m;
