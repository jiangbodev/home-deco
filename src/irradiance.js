...
export function createIrradiance(renderer){
 ...
 const maps={wall:{value:neutral},floor:{value:neutral}},weight={value:0},receivers=new Map(),receiverWeights=new Map(),seen=new WeakSet();let manifest,available=false,target=0;
 ...
 function prepare(group){group.traverse(o=>{
  ...
  const receiverWeight={value:1};receiverWeights.set(o,receiverWeight);
  const multi=Array.isArray(o.material);const materials=(multi?o.material:[o.material]).map(original=>{
   ...
   m.onBeforeCompile=(shader,...args)=>{
    previous.call(m,shader,...args);Object.assign(shader.uniforms,{tracedMap:maps[r.kind],tracedWeight:weight,tracedReceiverWeight:receiverWeight});
    ...
    shader.fragmentShader=varyings+'uniform sampler2D tracedMap; uniform float tracedWeight; uniform float tracedReceiverWeight;\n'+(r.kind==='wall'?'uniform vec3 tracedMin;uniform vec3 tracedSize;uniform vec4 tracedRects[6];\n':'uniform vec4 tracedBounds;\n')+shader.fragmentShader;
...
vec3 ta=abs(tn),tp=clamp((tracedPosition-tracedMin)/tracedSize,0.0,1.0);int tf;vec2 tu;
    ...
    shader.fragmentShader=shader.fragmentShader.replace('vec3 totalDiffuse = reflectedLight.directDiffuse + reflectedLight.indirectDiffuse;',`${uv}
     ...
     vec3 totalDiffuse=mix(reflectedLight.directDiffuse+reflectedLight.indirectDiffuse,diffuseColor.rgb*(tracedIrradiance*1.65+vec3(0.08)),tracedWeight*tracedReceiverWeight*tracedSample.a*${r.kind==='wall'?'0.72':'1.0'});`);
   };m.customProgramCacheKey=()=>key+'|cycles-diffuse-v3-'+r.kind;return m;
  });o.material=multi?materials:materials[0];
 })}
 function update(model,initialTransforms,loaded,assetFiles){let unchanged=true;model.traverse(o=>{if(!Object.keys(o.userData).some(k=>k.startsWith('interaction_')&&k!=='interaction_doors'&&k!=='interaction_bath-partition'&&!k.startsWith('interaction_shower-')))return;const initial=initialTransforms.get(o);if(!initial||initial.visible!==o.visible||o.matrix.elements.some((v,i)=>Math.abs(v-initial.matrix.elements[i])>1e-5))unchanged=false});for(const[o,w]of receiverWeights){w.value=1;for(let p=o;p&&p!==model;p=p.parent){const initial=initialTransforms.get(p);if(initial&&(initial.visible!==p.visible||p.matrix.elements.some((v,i)=>Math.abs(v-initial.matrix.elements[i])>1e-5))){w.value=0;break}}}target=available&&unchanged&&loaded?.length===assetFiles?.length&&JSON.stringify(manifest?.sourceModules)===JSON.stringify(assetFiles)?1:0;if(!target)weight.value=0}
